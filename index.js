...
if (process.env.SS_POLL_ENABLED === "1") {
  const iv = Math.max(60, Number(process.env.SS_POLL_INTERVAL_SEC || 180)) * 1000;
  if (!SS_POLL_LAST_ISO) {
    const d = new Date(Date.now() - 24 * 60 * 60 * 1000);
    SS_POLL_LAST_ISO = d.toISOString().slice(0, 19);
  }
  setInterval(async () => {
    try {
      const since = SS_POLL_LAST_ISO;
      const r = await processShipmentsSince(since, 5);
      SS_POLL_LAST_ISO = new Date().toISOString().slice(0, 19);
    } catch (_) {}
  }, iv);
}
